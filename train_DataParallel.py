# 성공 하지만 Custom은 에러남 뭔가 이상한듯 안하는게 나은듯
# 걍 이 바닐라로 하는게 좋아보임

# 에러 https://discuss.pytorch.org/t/mask-rcnn-nn-dataparallel-possible/53265/4
...
def train(args):
    ...
    if network == "CycleGAN":
        ...
        netG_a2b = CycleGAN(in_channels=nch, out_channels=nch, nker=nker, norm=norm, nblk=9)
        netG_b2a = CycleGAN(in_channels=nch, out_channels=nch, nker=nker, norm=norm, nblk=9)
        netG_a2b = torch.nn.DataParallel(netG_a2b)
        netG_b2a = torch.nn.DataParallel(netG_b2a)
        netG_a2b.to(device)
        netG_b2a.to(device)

        netD_a = Discriminator(in_channels=nch, out_channels=1, nker=nker, norm=norm)
        netD_b = Discriminator(in_channels=nch, out_channels=1, nker=nker, norm=norm)
